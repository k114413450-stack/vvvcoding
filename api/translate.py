    ...
    def do_POST(self):
        ...
        try:
            ...
            system_prompt = (
                ...
                "=== CRITICAL: SELL DIRECTION HANDLING ===\n"
                "When direction='SELL':\n"
                "- first_sell: entry condition for SHORT (e.g. '3 consecutive bearish candles' → 'close<open && close[1]<open[1] && close[2]<open[2]')\n"
                "- first_buy: set to null or empty string\n"
                "- exit_all: MUST ALWAYS be generated. Examples for SELL:\n"
                "  * If user says 'SELL when X, close when price drops 5 points' → exit_all: 'close >= last_entry + 5' (price rises = loss for SHORT)\n"
                "  * If user says 'SELL and TP 10, SL 5' → exit_all: 'close <= last_entry - 10 || close >= last_entry + 5'\n"
                "  * Default (no condition given): 'close > open' (exit when bullish candle appears)\n"
                "- Remember: For SELL positions, profit = entry_price - exit_price (inverse of BUY)\n\n"

                "=== POSITION-LEVEL PARAMETERS EXTRACTION ===\n"
                "Extract from text patterns like:\n"
                "- '止盈5点' or 'TP 5' → pos_tp: 5\n"
                "- '止损10点' or 'SL 10' → pos_sl: 10\n"
                "- '每单独立止盈' or '每单止盈' → separately applied TP per order\n"
                "- '每单独立止损' or '每单止损' → separately applied SL per order\n"
                "- '止盈5点，止损10点' → pos_tp: 5, pos_sl: 10\n"
                "IMPORTANT: Extract ALL numeric values after these keywords!\n\n"
                ...
                "- pos_sl: stop loss distance (in price units/dollars, e.g. 5 for $5 stop loss per order). EXTRACT from '每单止损5' or '止损5点' patterns.\n"
                "- pos_tp: take profit distance (in price units/dollars, e.g. 10 for $10 take profit per order). EXTRACT from '每单止盈10' or '止盈10点' patterns.\n"
                "- pos_trail_trigger: activation threshold for individual trailing stop profit (e.g. 5 for $5 profit activation).\n"
                "- pos_trail_callback: pullback callback distance for individual trailing stop profit (e.g. 2 for $2 callback).\n"
                "IMPORTANT: When pos_tp or pos_sl are present, DO NOT also generate exit_all with the same condition; they are independent mechanisms.\n\n"
